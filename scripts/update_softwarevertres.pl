...
foreach my $repo ("$base/bin-external", "$base/codebase", "$base/vrpipe/master", "$base/update_pipeline", "$base/conf", "$base/update_pipeline_hipsci") {
    foreach my $server ('uk10k-login', 'vr-login') {

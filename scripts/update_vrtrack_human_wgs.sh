...
date="`date +'%y%m%d'`"
mysqldump -u $VRTRACK_RW_USER -p$VRTRACK_PASSWORD -P$VRTRACK_PORT -h$VRTRACK_HOST vrtrack_human_wgs > "$DUMPS/vrtrack_human_wgs_$date.sql"

$BIN_EXT/update_pipeline.pl -s $CONF/human_wgs_studies -d vrtrack_human_wgs -v

$SCRIPTS/vrtrack_individual_supplier_name -d vrtrack_human_wgs

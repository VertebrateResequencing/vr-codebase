...
# density.R script for $opts{t} is in /software/vertres/vrpipe/
...
getopts('q:n:c:b:p:hmf:gCt:', \%opts);
die("
...
Options:
         ...
         -t STRING Turn on alternative cutoff calculation; provide full path of density.R script
...
foreach my $fbam(@ARGV){
  ...
  while(<BAM>){
    ...
    if(/^\@RG/){  #getting RG=>LIB mapping from the bam header
      ...
      next unless(($t->{flag}==18 || $t->{flag}==20) && $t->{dist}>=0 && $t->{dist}<100000);
  ...
  foreach my $lib(keys %insert_stat){  # stats for each library
	...
	##my $median=$insert_stat{$lib}->median();

    delete $insert_stat{$lib};
    my $insertsize=Statistics::Descriptive::Full->new();
    foreach my $x(@isize){  # use only reasonable insert sizes
      next if($x>$mean+10*$std);
      $insertsize->add_data($x);
    }

    $mean=$insertsize->mean();               # get new mean
    $std=$insertsize->standard_deviation();  # get new SD
    next if($mean<$opts{s});                 # skip if less than min. insert size
    my $cv=$std/$mean;
    next if($cv>=$opts{v});

    my $num=$insertsize->count();            # get new total n, must be >=100
    next if($num<100);

    my ($stdm,$stdp)=(0,0);
    my ($nstdm,$nstdp)=(0,0);
    foreach my $x($insertsize->get_data()){  # for each insert size...SD of upper and lower
      if($x>$mean){             # upper boundary
        $stdp+=($x-$mean)**2;   # (value - mean) squared
        $nstdp++;               # count total
      }
      else{                     # lower boundary
        $stdm+=($x-$mean)**2;
    ...
    $stdm=sqrt($stdm/($nstdm-1)); # SD
    $stdp=sqrt($stdp/($nstdp-1)); # SD
  ...
  foreach my $rg(keys %RGlib){
    ...
    my $upper=$mean+$opts{c}*$stdps{$lib} if(defined $opts{c}); # upper insert size cutoff
    my $lower=$mean-$opts{c}*$stdms{$lib} if(defined $opts{c});
    $lower=0 if(defined $lower && $lower<0);
    # Alternative method to calculate upper boundary:
    if ($opts{t}) {
		my $median = $insert_stat{$lib}->median();
		my @data = $insert_stat{$lib}->get_data();
		my $mad = mad($median,\@data);
		my $thresh1 = $median + 4*$std;
		my $thresh2 = $median + 5*$mad;
		my $thresh3 = densitythresh(@data);
        my @sorted = sort {$b<=>$a} ($thresh1,$thresh2,$thresh3); # get the highest number
        $upper = shift @sorted; # get the highest number
		##print STDERR "$lib: MED $median MAD $mad SD $std THRESH $thresh1, $thresh2, $thresh3, $upper\n";
...
#### get thresh from density function
...
sub densitythresh {
	...
	print F join "\n", @data;
	close F;
    my $thresh = `R --slave --vanilla --args inserts.bd < $opts{t}`;
    chomp $thresh if $thresh;
	return $thresh || '0';

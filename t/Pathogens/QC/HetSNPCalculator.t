...
throws_ok { $hsc->total_number_of_snps } qr/Backtrace:/, 'Total number of SNPs file doesnt exist yet';
...
my $het_report_master_file = File::Spec->catfile('t/data','15360_1#1_heterozygous_snps_report_master.txt');

is(compare($hsc->temp_vcf,$temp_master_file),0, 'Temp vcf file');
is(compare($hsc->snp_called_vcf,$snp_called_master_file),0, 'Snp called vcf file');
is(compare($hsc->filtered_snp_called_csv,$filtered_master_file),0, 'Filtered csv file');
is(compare($hsc->total_number_of_snps_csv,$total_number_of_snps_master_file),0, 'Total number of snps (Het and Hom)');

ok ( $hsc->write_het_report, 'Writing heterozygosity report' );
is ( $hsc->het_report_path, 't/data/15360_1#1_heterozygous_snps_report.txt', 'Het report path' );
is(compare($hsc->het_report_path,$het_report_master_file),0, 'Heterozygosity report file');

open (my $fh2, '<', $hsc->het_report_path) or die "Couldn't open the het report file for reading";
my @lines = <$fh2>;
close($fh2);
...
is ( $values[1], '4.55588960350548e-05', 'Percentage of heterozygous SNPs for the whole genome');
is ( $values[2], '0.00591961167347422', 'Percentage of heterozygous SNPs for all the SNPs called (Het and Hom)');

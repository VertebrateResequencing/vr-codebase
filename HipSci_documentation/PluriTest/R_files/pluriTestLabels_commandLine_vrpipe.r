...
pluriTestLabelsCommandLine <- function (NewDataFileName, DataRepository) 
{
    require(lumi)
    require(xtable)
    require(GO.db)
    ...
    try({
        ...
        filled.contour2(y = c(-129:70), x = c((1:200)/50), background129_70x1_4, 
            ...
            ylab = "pluripotency")
        labelsTmp <- names(s.new)
        sNewNames <- names(s.new)
        novelNewNames <- names(novel.new)
        namesAccessS <- s.new[sNewNames]
        namesAccessNovel <- novel.new[novelNewNames]
        tVectS <- as.vector(namesAccessS)
        tVectN <- as.vector(namesAccessNovel)

        textxy(tVectN, tVectS, labs=labelsTmp, cx=0.7)
        points(s.new ~ novel.new, cex = 0.4, main = "Overview") # THIS IS IT
...
pluriTestLabelsCommandLine(args[1], args[2])

...
{
	my ($self) = @_;
	my $tabix_cmd = 'cd '.$$self{'index_directory'}.' ; '.$$self{'tabix_location'}.' '.$$self{'svs_location'}.'  '.$$self{'pos_str'};
...
{
    my ($self) = @_;
    die("Sorry, no matching structural variations.\n");
}
...
sub print_legend
{
    ...
    my $lookseq_params = 
        "show=$$self{chrm}:$$self{from}-$$self{to},indel" .
        "&amp;win=$lookseq_win" .

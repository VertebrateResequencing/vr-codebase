...
# Loads info from an interval list file into a hash.
# Arg 1 = name of interval list file
# Arg 2 = ref_len = hash of reference lengths
# Arg 3 = ref to has of options. Fill in mmandatory options...
#  
# Resulting hashes look like:
# ref_id => [(start1, end1), (start2, end2), ...], where each list is sorted
# by start position then end position (see sub interval_compare)
#
# Full description of 'interval list' file here:
# http://picard.sourceforge.net/javadoc/net/sf/picard/util/IntervalList.html
sub _interval_list_file2hash
{
    my $infile = shift;
    my $ref_lens = shift;
    my $ops = shift;
    ...
    while (my $line = <$fh>)
    {
        next if ($line =~ m/^@/); # ignore header lines
        chomp $line;
        my @data = split /\t/, $line;
        # Y 21350782    21350939    +   CCDS44028.1.6
        
        # do we care about the current chromosome?
        next unless $data[0] =~ m/^$ops->{chr_regex}$/;

        # make sure reference id is in the hashes
        unless (exists $hit{$data[0]})
        {
            $hit{$data[0]} = ();
            $near{$data[0]} = ();
        }
        
        # add each (start, end) to the arrays
        push @{$hit{$data[0]}}, [$data[1], $data[2]];
        my $start = max(1, $data[1] - $ops->{near});
        my $end = min($ref_lens->{$data[0]}, $data[2] + $ops->{near});
        push @{$near{$data[0]}}, [$start, $end];
    }
    ...
    {
        @{$hit{$key}} = sort interval_compare @{$hit{$key}};
        @{$near{$key}} = sort interval_compare @{$near{$key}};
        merge_overlapping_intervals($hit{$key});
        merge_overlapping_intervals($near{$key});
    }
...
# Takes a reference to an array of intervals
# [[start1, end1], [start2, end2], ... ]
# and changes it so that overlapping
# or adjacent intervals are merged together.
# Assumes that the array is sorted, by start positionm then
# by end position.  e.g. [1,2] < [1,3] < [2,4]
sub _merge_overlapping_intervals
{
    my $l = shift;
    my $i = 0;
    ...
    while ($i < scalar  @$l - 1)
    {
        #print "$i, $l->[$i][0], $l->[$i][1], $l->[$i+1][0], $l->[$i+1][1]\n";
        # if current interval overlaps next interval, or is adjacent to it
        if ($l->[$i][1] >= $l->[$i+1][0] - 1)
        {
            $l->[$i + 1] = [$l->[$i][0], max($l->[$i][1], $l->[$i + 1][1])];
            splice @$l, $i, 1;
        }
        else
        {

...
=head2 add_sam_header
 ...
 Usage   : $obj->add_sam_header('headerless.sam', 'sequence.index');
 ...
 Args    : path to sam and sequence.index files (generated by DCC), and either
           just lane_path => '/path/to/lane' (to calculate the below using
           HierarchyUtilities::lane_info)
           -or-
...
sub add_sam_header {
    my ($self, $raw_sam_file, $seq_index, %args) = @_;
    ...
    my $parser = VertRes::Parser::sequence_index->new(file => $seq_index);
    my $individual = $parser->lane_info($lane, 'sample_name') || $self->throw("Unable to get sample_name for $lane from $seq_index");
    my $run_name = $parser->lane_info($lane, 'run_name');
    my $library = $parser->lane_info($lane, 'LIBRARY_NAME');
    my $platform = $parser->lane_info($lane, 'INSTRUMENT_PLATFORM');
    my $centre = $parser->lane_info($lane, 'CENTER_NAME');
    my $insert_size = $parser->lane_info($lane, 'INSERT_SIZE');

...
sub store_nfs 
{
    ...
    LSF::run($action_lock, $path, $job_name, $self, 
             qq{perl -MVertRes::Pipelines::StorePath -Mstrict -e "VertRes::Pipelines::StorePath->new()->store_path(qq[$path], qq[$storage_path]) || die qq[Failed to store path, $path in $storage_path\n]; system(qq[touch $done_file]);"});

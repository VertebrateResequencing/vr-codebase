...
our @actions = (
    ...
    # Runs glf to check the genotype. Inherited from TrackQC_Bam.
    {
        ...
        'requires' =>
          \&VertRes::Pipelines::TrackQC_Bam::check_genotype_requires,
        'provides' =>
...
our $options = {
    ...
    'adapters' =>
      '/software/pathogen/projects/protocols/ext/solexa-adapters.fasta',
    'bsub_opts' => "-q normal -M5000 -R 'select[mem>5000] rusage[mem=5000]'",
    'bsub_opts_heterozygous_snps' =>
      "-q normal -M500 -R 'select[mem>500] rusage[mem=500]'",
    'bsub_opts_stats_and_graphs' =>
      "-q normal -M1000 -R 'select[mem>1000] rusage[mem=1000]'",
    'bsub_opts_map_sample' =>
      "-q normal -M5000 -R 'select[mem>5000] rusage[mem=5000]'",
    'bsub_opts_process_fastqs' =>
      "-q normal -M3200 -R 'select[mem>3200] rusage[mem=3200]'",
    'bsub_opts_subsample' =>
"-q normal -M1000 -R 'select[mem>1000] rusage[mem=1000]' -n 2 -R 'span[hosts=1]'",
    'bwa_clip'         => 20,
    'chr_regex'        => '^(?:\d+|X|Y)$',
    'gc_depth_bin'     => 20000,
...
sub rename_files {
    ...
    if ( scalar @files > 3 ) {
        Utils::error(
            "FIXME: so far can handle up to 3 fastq files: $lane_path.\n");
    }
...
#   paired, which are non-paired and if they should be used at all.
            $self->throw(
qq[Heuristic failed, did not expect "$file". Please symlink manually.]
            );
        }
        elsif ( $i < 3 && !( $file =~ /_\d+\D*$/ ) && scalar @files > 1 ) {
            $self->throw(
qq[Heuristic failed, which files are single-ended and which are paired? Please symlink manually.]
            );
        }
        elsif ( $i == 3 && ( $file =~ /_\d+\D*$/ ) ) {
            $self->throw(
qq[Heuristic failed, this should be single-ended file "$file". Please symlink manually.]
            );
        }
        ...
        if ( -e "$file.fastqcheck"
            && !-e "$lane_path/${name}_$i.fastq.gz.fastqcheck" )
        {
            Utils::relative_symlink( "$file.fastqcheck",
                "$lane_path/${name}_$i.fastq.gz.fastqcheck" );
        }
        if ( -e "$file.md5" && !-e "$lane_path/${name}_$i.fastq.gz.md5" ) {
            Utils::relative_symlink( "$file.md5",
                "$lane_path/${name}_$i.fastq.gz.md5" );
        }
...
sub cmp_last_number($$) {
    ...
    if ( !( $a =~ /_(\d+)\D*$/ ) ) {
        return 1;
    }    # is greater than the rest if there is no number in the name
    my $x = $1;
    if ( !( $b =~ /_(\d+)\D*$/ ) ) { return -1 }
    my $y = $1;
    return $x <=> $y;
...
sub subsample {
    ...
    open( my $fh, '>', "$lane_path/$sample_dir/_qc-sample.pl" )
      or $self->throw("$lane_path/$sample_dir/_qc-sample.pl: $!");
...
#
    ...
    for ( my $i = $nfiles ; $i > 0 ; $i-- ) {
        ...
        print $fh
"FastQ::sample(q{../${name}_$i.fastq.gz},q{${name}_$i.fastq.gz}, $$self{'sample_size'}$print_seq_list);\n";
    }
    close $fh;

    VertRes::LSF::run(
        $lock_file, "$lane_path/$sample_dir", "_${name}_sample",
        { bsub_opts => $$self{bsub_opts_subsample} },
        qq{perl -w _qc-sample.pl}
    );
...
sub process_fastqs {
    ...
    if ( $nfiles < 1 || $nfiles > 3 ) {
        Utils::error(
            "FIXME: we can handle 1-3 fastq files in $work_dir, not $nfiles.\n"
        );
    }
    ...
    for ( my $i = 1 ; $i <= $nfiles ; $i++ ) {
        ...
        open( my $fh, '>', "$work_dir/${prefix}aln_fastq_$i.pl" )
          or Utils::error("$work_dir/${prefix}aln_fastq_$i.pl: $!");
        print $fh qq[
use strict;
use warnings;
use Utils;
...
rename("${name}_$i.saix","${name}_$i.sai") or Utils::error("rename ${name}_$i.saix ${name}_$i.sai: \$!");
        ...
        close($fh);
        VertRes::LSF::run(
            $lock_file, $work_dir, "_${name}_$i",
            { bsub_opts => $$self{bsub_opts_process_fastqs} },
            qq[perl -w ${prefix}aln_fastq_$i.pl]
        );
    }

# Run blat for each fastq file to find out how many adapter sequences are in there.
    Utils::CMD(
        qq[cat $$self{adapters} | sed 's/>/>ADAPTER|/' > $work_dir/adapters.fa]
    );

    for ( my $i = 1 ; $i <= $nfiles ; $i++ ) {
        if ( -e qq[$work_dir/${name}_$i.nadapters] ) { next; }

        open( my $fh, '>', "$work_dir/${prefix}blat_fastq_$i.pl" )
          or Utils::error("$work_dir/${prefix}blat_fastq_$i.pl: $!");
        print $fh qq[
use strict;
use warnings;
use Utils;
...
unlink("${name}_$i.fa", "${name}_$i.blat");
        ...
        close($fh);
        VertRes::LSF::run(
            $lock_file, $work_dir, "_${name}_a$i",
            { bsub_opts => $$self{bsub_opts_process_fastqs} },
            qq[perl -w ${prefix}blat_fastq_$i.pl]
        );
    }
...
sub map_sample {
    ...
    my $samtools = $$self{'samtools'};
    my $max_isize =
      exists( $$self{insert_size} ) ? $$self{insert_size} * 2 : 2000;
...
#
    ...
    for my $single (@singles) {
        if ( !( $single =~ m{/?(${name}_\d+).fastq.gz$} ) ) {
            Utils::error("Failed to parse [$single]");
        }
        my $sname   = $1;
...
rename("${sname}.bam.part", "$sname.bam") or Utils::error("rename ${sname}.bam.part $sname.bam: \$!");
    ...
    if ( scalar @paired ) {
        my $bwa_cmd =
"$bwa sampe -a $max_isize $bwa_ref ${name}_1.sai ${name}_2.sai ${name}_1.fastq.gz ${name}_2.fastq.gz";
...
rename("x$name.bam","$name.bam") or Utils::error("rename x$name.bam $name.bam: \$!");
        ...
        print $fh
qq[rename('$single_bams[0]',"$name.bam") or Utils::error("rename $single_bams[0] $name.bam: \$!");\n];
    }
    close($fh);

    VertRes::LSF::run(
        $lock_file, $work_dir, "_${name}_sampe",
        { bsub_opts => $$self{bsub_opts_map_sample} },
        q{perl -w _map.pl}
    );
    return $$self{'No'};
}
...
system("touch $lane_path/_heterozygous_snps_done") and die "Error touch $lane_path/_heterozygous_snps";
    ...
    VertRes::LSF::run(
        $lock_file,
        "$lane_path/$sample_dir",
        "_heterozygous_snps",
        { bsub_opts => $self->{bsub_opts_heterozygous_snps} },
...
sub stats_and_graphs {
    ...
    my $name = $$self{lane};
    my $vrlane = VRTrack::Lane->new_by_hierarchy_name( $vrtrack, $name )
      or $self->throw("No such lane in the DB: [$name]\n");
    my $insert_size =
      ( VRTrack::Library->new( $vrtrack, $vrlane->library_id() )->insert_size()
      ) * 3
      || 8000;

    # Dynamic script to be run by LSF.
    open( my $fh, '>', "$lane_path/$sample_dir/_graphs.pl" )
...
\$qc->run_graphs(\$params{lane_path}, $reference_size, $insert_size);
    ...
    VertRes::LSF::run(
        $lock_file, "$lane_path/$sample_dir", "_${lane}_graphs",
        { bsub_opts => $$self{bsub_opts_stats_and_graphs} },
        qq{perl -w _graphs.pl}
    );
    return $$self{'No'};
}
...
sub run_graphs {
    ...
    my $bamcheck_file =
      qq[$lane_path/$$self{'sample_dir'}/$$self{'lane'}.bam.bc];
    my $cover_file = qq[$lane_path/$$self{'sample_dir'}/$$self{'lane'}.cover];

    my $genomecover = Pathogens::Parser::GenomeCoverage->new(
    ...
    # Output cover file.
    open( my $cov_fh, "> $cover_file" )
      or $self->throw("Cannot open: $cover_file\n");

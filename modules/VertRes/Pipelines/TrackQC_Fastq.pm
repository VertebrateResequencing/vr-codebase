...
our $options = 
{
    ...
    'bsub_opts_stats_and_graphs'   => "-q normal -M1000000 -R 'select[type==X86_64] select[mem>1000] rusage[mem=1000,thouio=1]'",
    'bsub_opts_map_sample'         => "-q normal -M1000000 -R 'select[type==X86_64] select[mem>1000] rusage[mem=1000,thouio=1]'",
    'bsub_opts_process_fastqs'     => "-q normal -M1000000 -R 'select[type==X86_64] select[mem>1000] rusage[mem=1000,thouio=1]'",
    'bsub_opts_subsample'          => "-q normal -M1000000 -R 'select[type==X86_64] select[mem>1000] rusage[mem=1000,thouio=1]'",
    'bwa_clip'        => 20,
    'chr_regex'       => '^(?:\d+|X|Y)$',
    'gc_depth_bin'    => 20000,

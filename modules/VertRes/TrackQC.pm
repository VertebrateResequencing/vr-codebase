...
=head2 new
        ...
        Options    : See Pipeline.pm for general options.
                    ...
                    insert_size     .. expected insert size
...
sub new 
{
    ...
    if ( !$$self{min_glf_ratio} ) { $self->throw("Missing the option min_glf_ratio.\n"); }
...
sub lane_info
{
    ...
    if ( !exists($$self{known_gtypes}) ) { return ''; }
    if ( !exists($$self{known_gtypes}{$project}) ) { return ''; }
    my $gtype = $$self{known_gtypes}{$project};

    if ( $field eq 'genotype' ) { return $gtype; }
    if ( $field eq 'gtype_confidence' )
    {
        if ( !exists($$self{gtype_confidence}) || !exists($$self{gtype_confidence}{$gtype}) ) { return ''; }
        if ( ref($$self{gtype_confidence}) eq 'HASH' )
        {
            return $$self{gtype_confidence}{$gtype};
        }
...
(
    ...
    'insert_size'  => q[$$self{insert_size}],
);
...
sub update_db
{
    ...
    $self->debug("status: $qc_status\n");
    if ( $qc_status ne 'no_qc' ) { return; }
...
=cut
    ...
    $mapping->genotype_ratio($$gtype{radio});
    ...
    for (my ($imgname,$caption) = each %images)
    {
        my $img = $mapping->add_image_by_name("$lane_path/$sample_dir/$imgname");
        $img->caption($caption);
        $img->update;
    }

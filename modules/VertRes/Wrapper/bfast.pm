...
sub setup_reference {
    ...
    my $existing_bifs = 0;
    for my $i (1..4) {
        my $bif = $ref.".nt.1.$i.bif";
        $existing_bifs += -s $bif ? 1 : 0;
    }
    
    unless ($existing_bifs == 4) {
        for my $i (1..4) {
            my $bif = $ref.".nt.1.$i.bif";
            unlink($bif);
        }
        ...
        my $i = 0;
        foreach my $mask (qw(111111111111111111
            ...
            $i++;
            $self->simple_run("index -f $ref -m $mask -d 1 -w 14 -i $i -n 1 -T /tmp/ -t");
        }
    }

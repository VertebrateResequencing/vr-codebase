...
VertRes::Wrapper::maq - wrapper for maq, special version for mapper comparison
...
package VertRes::Wrapper::maq_mapper_comparison;
...
sub do_mapping {
    ...
    # create new versions of fastqs with short ids, and a lookup file
    my $lookup_file = $out_sam.'.lookup';
    my $already_converted = 0;
    foreach my $fq (@fqs) {
        my $simplified_file = $fq.'.simplified';
        if (-e $simplified_file) {
            $already_converted++;
        }
    }
    unless ($already_converted == @fqs) {
        foreach my $fq (@fqs) {
            my $simplified_file = $fq.'.simplified';
            unless (-e $simplified_file) {
                my $i = VertRes::IO->new(file => $fq);
                my $new_fq = $fq;
                $new_fq =~ s/\.fastq.gz$/.fastq.simple_ids.gz/;
                my $o = VertRes::IO->new(file => ">$new_fq");
                my $ifh = $i->fh;
                my $ofh = $o->fh;
                
                my $lfh;
                unless (-s $lookup_file) {
                    open($lfh, '>', $lookup_file) || $self->throw("Could not write to lookup file '$lookup_file'");
                }
                
                my $lookup_id = 0;
                my $expected_lines = 0;
                while (<$ifh>) {
                    if ($expected_lines % 4 == 0) {
                        $lookup_id++;
                        s/^@([^\/]+)\/(\d)/\@lu_$lookup_id\/$2/;
                        print $lfh "lu_$lookup_id $1\n" if $lfh;
                    }
                    $expected_lines++;
                    print $ofh $_;
                }
                ...
                $i = VertRes::IO->new(file => $new_fq);
                my $actual_lines = $i->num_lines;
                unless ($actual_lines == $expected_lines) {
                    unlink($new_fq);
                    $self->throw("tried to make $new_fq, but it had $actual_lines instead of $expected_lines lines\n");
                }
                else {
                    unlink($fq);
                    move($new_fq, $fq) || $self->throw("could not move $new_fq to $fq");
                    system("touch $simplified_file");
                }
                
                if ($lfh) {
                    $i = VertRes::IO->new(file => $lookup_file);
                    my $actual_lines = $i->num_lines;
                    $expected_lines /= 4;
                    unless ($actual_lines == $expected_lines) {
                        unlink($lookup_file);
                        $self->throw("tried to make $lookup_file, but it had $actual_lines instead of $expected_lines lines\n");
                    }
                }
            }
        }
    ...
    unless (-s $out_map) {
        ...
        unless (defined $args{e} && defined $args{1} && (@fqs > 1 ? defined $args{2} : 1)) {
            ...
            foreach my $fqcp (@fqcps) {
                push(@read_lengths, $fqcp->max_length);
                $i++;
            }

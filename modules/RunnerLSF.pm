...
sub new
{
    ...
    $$self{default_limits} = { runtime=>40*60, memory=>1_000, queue=>'normal' };
    $$self{queue_limits}   = { basement=>1e9, long=>2880*60, normal=>720*60, small=>30*60 };
...
sub _parse_bjobs_l
{
    ...
    for (my $i=0; $i<@lines; $i++)
    {
        ...
        elsif ( $lines[$i]=~/The CPU time used is (\d+) seconds./ ) 
        { 

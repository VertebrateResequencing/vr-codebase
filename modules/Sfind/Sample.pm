...
sub _get_library_request_ids {
    ...
    my $sql = qq[select cr.internal_id from current_requests cr, current_aliquots a where ((request_type like '%library creation' or request_type like '%library preparation') or (request_type like '%Pulldown WGS' or request_type like '%Pulldown SC' or request_type like '%Pulldown ISC')) and source_asset_internal_id = a.receptacle_internal_id and a.sample_internal_id=? and cr.study_internal_id=?];
    my @library_requests;

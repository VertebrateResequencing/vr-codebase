...
my %opts = (l=>36, 1=>0, 2=>0, 3=>0, 4=>0, a=>250, n=>2000000, d=>'', A=>0,
			R=>'select[mem>1500 && type==X86_64] rusage[mem=1500]');
getopts('l:1:2:a:n:sR:zd:S3:4:f:A:', \%opts);
die(qq(
...
my $suffix = '';
for my $f (@ARGV) {
  $f =~ /\.(\w+)(\.gz)?$/ or die "[maq_sanger] Do not recognise suffix of $f\n";
  if ($suffix){
    unless ($suffix eq $1){
	die "[maq_sanger] read file suffixes must be identical\n";
    }
  }
  else {
    $suffix = $1;
...
bsub -w "done($jn.cat)" -o /dev/null -e /dev/null <<EOF
  find . -name "*.$suffix" | sed "s/\.$suffix\$//" | xargs -i echo '$maq fastq2bfq -n $opts{n} {}.$suffix {}' | $asub -j "$jn.bfq" $opts{R};
...
cd $cwd;
  ...
  if (defined $opts{z}) {
	$catfilter = qq(
ls *.$suffix.gz | sed "s/\.gz\$//" | xargs -i echo 'gzip -dc {}.gz | $maq catfilter -s - $sol2sanger > {}' | $asub -j "$jn.cat" $opts{R};\n);
  } else {
	$catfilter = qq(
ls *.$suffix | xargs -i mv {} {}.tmp;
